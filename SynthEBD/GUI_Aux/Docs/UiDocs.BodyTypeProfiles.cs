...
public static partial class UiDocs
{
    private static void RegisterBodyTypeProfiles()
    {
        ...
        Add("BodyTypeProfiles.MatchShowClosestAssignment",
            layperson: "Extends each label's line in the results tooltips with its 'next closest': the label that preset would have received instead, and how far it sits from that outcome.",
            technical: "ShowMatchPresetClosestAssignment. Each row's tooltip already lists every assigned descriptor with its margin score, one category per line; this appends to that same line the best-scoring value the row did NOT receive plus the gap between the two, in the unit the sort mode uses (standard deviations unless a percent-of-threshold sort is active). Scoring is deferred to hover, so only rows actually inspected are computed.",
