...
public static partial class UiDocs
{
    private static void RegisterBodySlides()
    {
        ...
        Add("BodySlides.HideAndDisableListButton",
            ...
            technical: "One-way action via VM_BodySlidePlaceHolder.HideAndDisable: sets BodySlideSetting.AllowRandom = false and, if not already hidden, HideInMenu = true (mirrored to the editor VM when the preset is selected). Same pairing the importer applies to outfit/refit presets. Presets in this hidden-and-disabled state (VM_BodySlidePlaceHolder.IsHiddenAndDisabled) are also left out of the Label by Measurements and Label by Sliders menus: preset pickers, scans, Match Presets, Rules-tab matches, Show Spread, histograms, the annotation table and the Compare window those menus open. The same HD button appears on the rows of those menus' preset lists (Match Presets, Rules-tab matches, annotation table, Show Spread cells, Label by Sliders preset browser); clicking one drops that preset's rows in place, and score badges re-normalize on the list's next rebuild.",
