...
public class VM_SelectableCollectionMemberString : VM
{
    public VM_SelectableCollectionMemberString(VM_CollectionMemberString subscribedString, ICollectionParent parentCollection)
    {
        ...
        this.SubscribedString.WhenAnyValue(x => x.Content).Skip(1).Subscribe(x => TriggerParentMemberChanged());
...
{
    public VM_CollectionMemberStringCheckboxList(ObservableCollection<VM_CollectionMemberString> masterList)
    {
        this.SubscribedMasterList = masterList;
        foreach (var cms in SubscribedMasterList)
        {
            this.CollectionMemberStrings.Add(new VM_SelectableCollectionMemberString(cms, this));
        }

        CollectionMemberChangedAction();
        this.WhenAnyValue(x => x.SubscribedMasterList).Subscribe(x => CollectionMemberChangedAction());
    }
