...
public class MainWindow_ViewModel : VM
{
    ...
    public MainState State { get; } = new();
    ...
    public MainWindow_ViewModel()
    {
        ...
        TexMeshSettingsVM = new VM_SettingsTexMesh(this, State);
    ...
    public void LoadInitialSettingsViewModels() // view models that should be loaded before plugin VMs
    {
        ...
        // load BlockList
        State.BlockList = SettingsIO_BlockList.LoadBlockList(out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        VM_BlockListUI.GetViewModelFromModel(State.BlockList, BlockListVM);
    ...
    public void LoadPluginViewModels()
    {
        ...
        State.BodyGenConfigs = SettingsIO_BodyGen.LoadBodyGenConfigs(PatcherSettings.General.RaceGroupings, out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        VM_SettingsBodyGen.GetViewModelFromModel(State.BodyGenConfigs, PatcherSettings.BodyGen, BodyGenSettingsVM, GeneralSettingsVM);

        VM_SettingsOBody.GetViewModelFromModel(PatcherSettings.OBody, OBodySettingsVM, GeneralSettingsVM.RaceGroupings);

        State.RecordTemplatePlugins = SettingsIO_AssetPack.LoadRecordTemplates(out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        State.RecordTemplateLinkCache = State.RecordTemplatePlugins.ToImmutableLinkCache();

        // load asset packs
        State.AssetPacks = SettingsIO_AssetPack.LoadAssetPacks(PatcherSettings.General.RaceGroupings, State.RecordTemplatePlugins, State.BodyGenConfigs, out loadSuccess); // load asset pack models from json
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        VM_AssetPack.GetViewModelsFromModels(State.AssetPacks, PatcherSettings.TexMesh, this); // add asset pack view models to TexMesh shell view model here

        // load heights
        State.HeightConfigs = SettingsIO_Height.LoadHeightConfigs(out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }

        VM_HeightConfig.GetViewModelsFromModels(HeightSettingsVM.AvailableHeightConfigs, State.HeightConfigs);
    ...
    public void LoadFinalSettingsViewModels() // view models that should be loaded after plugin VMs because they depend on the loaded plugins
    {
        ...
        State.SpecificNPCAssignments = SettingsIO_SpecificNPCAssignments.LoadAssignments(out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        VM_SpecificNPCAssignmentsUI.GetViewModelFromModels(SpecificAssignmentsUIVM, State.SpecificNPCAssignments, OBodySettingsVM, GeneralSettingsVM, this);

        // Load Consistency (must load after plugin view models)
        State.Consistency = SettingsIO_Misc.LoadConsistency(out loadSuccess);
        if (!loadSuccess) { Logger.SwitchViewToLogDisplay(); }
        VM_ConsistencyUI.GetViewModelsFromModels(State.Consistency, ConsistencyUIVM.Assignments, TexMeshSettingsVM.AssetPacks);
    }

    public void SavePluginViewModels()
    {
        VM_AssetPack.DumpViewModelsToModels(TexMeshSettingsVM.AssetPacks, State.AssetPacks);
        VM_HeightConfig.DumpViewModelsToModels(HeightSettingsVM.AvailableHeightConfigs, State.HeightConfigs);
        VM_SettingsBodyGen.DumpViewModelToModel(BodyGenSettingsVM, PatcherSettings.BodyGen, State.BodyGenConfigs);
    }
    ...
    public void DumpViewModelsToModels()
    {
        ...
        VM_SpecificNPCAssignmentsUI.DumpViewModelToModels(SpecificAssignmentsUIVM, State.SpecificNPCAssignments);
        VM_BlockListUI.DumpViewModelToModel(BlockListVM, State.BlockList);
        VM_ConsistencyUI.DumpViewModelsToModels(ConsistencyUIVM.Assignments, State.Consistency);
    ...
    void MainWindow_Closing(object sender, CancelEventArgs e)
    {
        ...
        SettingsIO_AssetPack.SaveAssetPacks(State.AssetPacks, out saveSuccess);
        ...
        SettingsIO_Height.SaveHeightConfigs(State.HeightConfigs, out saveSuccess);
        if (!saveSuccess) { allExceptions += exceptionStr + Environment.NewLine; showFinalExceptions = true; }
        ...
        SettingsIO_BodyGen.SaveBodyGenConfigs(State.BodyGenConfigs.Female, out saveSuccess);
        if (!saveSuccess) { allExceptions += "Error saving BodyGen configs" + Environment.NewLine; showFinalExceptions = true; }

        SettingsIO_BodyGen.SaveBodyGenConfigs(State.BodyGenConfigs.Male, out saveSuccess);
        if (!saveSuccess) { allExceptions += "Error saving BodyGen configs" + Environment.NewLine; showFinalExceptions = true; }
        ...
        SettingsIO_Misc.SaveConsistency(State.Consistency, out saveSuccess);
        if (!saveSuccess) { allExceptions += "Error saving Consistency" + Environment.NewLine; showFinalExceptions = true; }

        SettingsIO_SpecificNPCAssignments.SaveAssignments(State.SpecificNPCAssignments, out saveSuccess);
        if (!saveSuccess) { allExceptions += "Error saving Specific NPC Assignmentss" + Environment.NewLine; showFinalExceptions = true; }

        SettingsIO_BlockList.SaveBlockList(State.BlockList, out saveSuccess);
