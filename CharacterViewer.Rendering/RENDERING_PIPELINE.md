...
**Translucent, depth-tested region solid.** `RegionSolidTriangles` has two opt-in knobs, both off by default so the editor's Solid view is unchanged: `RegionSolidAlpha < 1` alpha-blends the surface (`u_alpha` = that value, depth writes off) with `glBlendFuncSeparate(SRC_ALPHA, ONE_MINUS_SRC_ALPHA, ZERO, ONE)` — the separate alpha factors keep the destination alpha at 1, so an offscreen BGRA readback doesn't come out semi-transparent where the tint sits; and `RegionSolidDepthTested` re-enables the depth test for this one draw with `glPolygonOffset(-1, -2)` on fill, because the region patch lies exactly on the body surface and would otherwise z-fight. Together they turn the region into a surface tint: shaded where the patch is visible, hidden on the far side, and caps buried inside the body never show. This works because the overlay pass draws into the scene FBO whose depth buffer still holds the body (the same reason the wireframe overlay can depth-test). `DrawRegionSolid` restores depth-off / opaque / depth-writes-on before the marker spheres draw.
...
| `RegionSolidTriangles` | `List<float>` interleaved pos+normal | magenta (`RegionSolidColor`); opt. translucent (`RegionSolidAlpha`) + depth-tested (`RegionSolidDepthTested`) | `DrawRegionSolid` (marker pass, lit) | `SetRegionSolid` |
