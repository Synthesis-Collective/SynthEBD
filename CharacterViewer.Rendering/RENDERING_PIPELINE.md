...
3. [Part 2 — Fragment shader pipeline](#part-2--fragment-shader-pipeline)
   ...
   - [Per-slot texture toggles & the visibility lock](#per-slot-texture-toggles--the-visibility-lock)
...
- **Interactive prewarm** ([VM_CharacterViewer.cs](ViewModels/VM_CharacterViewer.cs) `PrewarmIdentityAsync` -> `CharacterPreviewCache.PrewarmNpc`): the same offload, reached from the live viewer instead of the offscreen batch path, for hosts that know which NPC they will show next while the user is still looking at the current one. It resolves mesh paths and runs the GL-free parse + decode on the thread pool and **does not touch the committed scene** - no `StatusText`, no `IsLoading`, no cancellation of an in-flight load - so it can cost time but never changes what is rendered. It no-ops when the identity is already the loaded one, swallows its own failures (a miss just loads lazily as before), and takes a caller-owned `CancellationToken` because a stale prediction competing for I/O with the load the user actually asked for is worse than no prefetch. SynthEBD reaches it through `CharacterViewerSynthEbdExtensions.PrewarmNpcAsync(FormKey, ILinkCache, ct)`; its first consumer is the Label-then-Suggest annotation queue, which warms the next queued slice's preview NPC. Because `LoadAsync` short-circuits on an unchanged identity, this only does real work when the next slice crosses a weight or gender boundary.
...
**`ApplyTextureOverrides(IEnumerable<TextureOverride>)`** — retargets texture slots (0/1/2/3/7) on existing skin shapes. Each `TextureOverride(bodyPart, slot, gameRelativePath)` routes to `Renderer.Meshes.Where(m => m.BodyPart == bodyPart && m.IsSkinShape)` (Head routes only to the primary head shape). The host determines `bodyPart`/`slot` by resolving its config destination path against the actual record — see "Host-side destination resolution" below — not by string-matching; `slot` comes from the terminal texture-set property (`ParseTextureSlot`). Slot 3 (facegen detail — a config routing to `HeadTexture.Height.GivenPath`) replaces the head's complexion/dirt detail map; if the override file doesn't resolve, the mesh's current slot-3 state is kept rather than binding the white fallback (same rationale as the [missing detail-map demotion](#detail-map-face)).

*Per-shape (worn-armor `AlternateTextures`).* A `TextureOverride` may carry an optional `ShapeName`. When set, it targets **only** the shape whose NIF geometry node name equals it — `Renderer.Meshes.Where(m => m.BodyPart == bodyPart && m.ShapeName == ShapeName)` — ignoring the skin-shape gate (an alternate texture can retexture a non-skin sub-shape). This is the **base-body** counterpart to the mesh-override `ShapeTextures` channel below: a worn-armor `AlternateTextures` (MODS) entry retextures one named sub-shape of a body NIF that a replacer split into several distinctly-named shapes, and the base body is rendered directly, not as a mesh override, so it needs this path rather than `MeshOverride.ShapeTextures`. Shape-named overrides are applied **after** the flat body-wide ones (stable `OrderBy` on `ShapeName` emptiness) so the more specific per-shape texture wins on its shape when a config carries both for the same slot. **Index vs. Name:** the alternate texture's `Index` field is *not* the NIF shape enumeration order (a `[Index == 1]` entry named `BodyShapeB` can be the third geometry in the file), so the host must resolve the entry's `Name` — the target 3D object name — from the record template (SynthEBD does this in `SubgroupTextureMapper` via `RecordPathParser.GetObjectAtPath`, walking the `AlternateTextures[…]` destination against the pack's record-template NPC) and pass that here; matching by geometry index alone would texture the wrong shape. The mesh-override channel's `MeshOverride.AlternateTextures` handles the same two-field problem from the other side: it carries **both** the entry's 3D Name and 3D Index, matches by name first, and only entries whose name matches *no* shape of the mesh fall back to index matching (`AlternateTextureMatching`) — because BodySlide/Outfit Studio rebuilds **rename** shapes while keeping their order (name goes stale, index stays good; the engine keys on the index, so the variant renders in game but was lost here — the “black skirt” bug), while block-sorting tools desync the index but keep names good. Renamers keep order and re-sorters keep names, so name-first + dangling-only index fallback covers both.
...
3. applies the bundled `Textures` (else the NIF's own `BSShaderTextureSet`), then folds any per-shape `AlternateTextures` (MODS) entries on top of their target shape — matched by 3D Name with a 3D-Index fallback for renamed/rebuilt meshes (see the *Per-shape* note above and `AlternateTextureMatching`); entries that bind to no shape by either key are logged, not silently dropped — a SynthEBD auxiliary subgroup ships mesh + its slot's `SkinTexture.*` together, so the texture set travels *with* the mesh rather than through the texture-only channel;
...
**Host-side destination resolution (SynthEBD).** How SynthEBD decides which `TextureOverride` / `MeshOverride` a selected subgroup produces is *not* the renderer's concern, but it's what makes the two channels above able to display **any** asset a config references. Rather than string-matching a fixed set of known destination shapes, `SubgroupTextureMapper` resolves each config destination path the way the patcher does — walking it against the pack's record-template NPC (falling back to the preview NPC) via `RecordPathParser` — and reads the target from the *actual resolved record*: the body part / biped slots come from the resolved `ArmorAddon`'s `BodyTemplate.FirstPersonFlags` (so a multi-slot armature emits one override per occupied base slot, and an auxiliary slot becomes a `Slot{n}` mesh override), the texture slot from the terminal `TextureSet` property, and a worn-armor `AlternateTextures` entry's target shape from its `Name`. A destination that doesn't resolve is skipped and logged — never mis-applied. This means a non-standard armature, a custom slot, or an armature selected by a condition that never names its slot all render correctly, where the previous substring heuristic silently dropped them.
...
### Per-slot texture toggles & the visibility lock

The viewer's right-click menu switches texture slots on and off per shape. Each `GlMesh` carries nine flags — `DiffuseEnabled`, `NormalEnabled`, `SkinEnabled`, `SpecularEnabled`, `FaceTintEnabled`, `DetailEnabled`, `EnvMapEnabled`, `EmissiveEnabled`, `TintColorEnabled`, all default `true` — pushed per draw as the `u_enable*` uniforms. `basic.frag` ANDs each with the stage's own `has_*` gate, so "off" skips that stage: Diffuse off substitutes flat `vec4(0.8, 0.8, 0.8, 1.0)`, Normal off falls back to the interpolated vertex normal (and loses the normal-alpha specular mask), Skin off drops SSS, and so on. Lighting, shadows and SSAO are geometry-driven and stay on, which is the point — with textures off only the mesh's own relief shows. `TextureSlots` ([Gl/TextureVisibilityLock.cs](Gl/TextureVisibilityLock.cs)) names the nine as flags; `GlMeshTextureSlotExtensions` reads and writes them, and `PresentTextureSlots` is the set a mesh actually has (the menu offers only those).

> **Diffuse off writes alpha 1.0.** An alpha-tested shape (hair cards, brows, lashes) therefore draws as solid grey sheets in the main pass, while `depth_only.frag` / `shadow_depth.frag` still cut out from the diffuse alpha regardless of the toggle — so their shadows and ambient occlusion keep the cut-out shape.

**The lock.** The flags live on each mesh, so any load that builds new meshes brings every slot back on — and the OBody annotation queue does that on every weight change, since each weight slot has its own preview NPC. "Lock Texture Visibility" keeps the choice at the viewer level: `VM_CharacterViewer.SetTextureVisibilityLocked(true)` snapshots the scene into a `TextureVisibilityLock`, and every mesh created afterwards gets the remembered choice. It is **session-only** (never persisted: an invisible, restored "textures off" would mislead), and additive: off by default, and NPC Plugin Chooser 2 never calls it.

- **Keys**, resolved per slot, most specific first: the **shape** (`BodyPart` + trimmed `ShapeName`, case-insensitive), then its **group** (`BodyPart` + kind — skin / eye / hair / other, from `IsSkinShape` / `IsEye` / `IsHairTintShader`), then a **fallback** for groups never seen, set only by "Hide All Textures" or by engaging on a fully hidden scene. The kind exists because a FaceGen head's face, eyes, hair and brows are all `BodyPart == "Head"`: a per-part rule would strip the next NPC's hair because this NPC's face was hidden. On engage, a shape remembers its whole state (on and off) but its group learns only what is off — on is every new mesh's default, and recording it would let one untouched shape cancel a hidden sibling.
- **Where it applies:** just before each `Renderer.AddMesh` — `InstallOneShapeTextures` (the sliced scene install), `ApplyOneMeshOverride`, `InstallReplacedHead`, `InstallGuestScene` — once textures, `BodyPart` and the shader flags are set. Deliberately *not* one sweep at `SceneCommitted`: the install is sliced across render ticks, so early shapes would draw textured for a few frames, and a sweep over meshes already on screen could override choices made on them. `ApplyTextureOverrides` re-applies to just the slots an override made present (a normal/skin/detail/specular map the NIF lacked).
- **A slot the mesh lacks is never touched** — toggling it is either invisible (the `has_*` gate) or wrong (Diffuse off greys an untextured shape).
- **Guest overlay:** guest meshes get the lock too, except `TintColor`, which the Translucent style draws its flat overlay colour through. The lock learns only from the primary scene: guest shapes share names with it and would overwrite its choices.
- **Menu semantics:** while locked, each slot checkbox also updates the remembered state (the lock follows the latest choice, not a frozen snapshot). "Hide All Textures" turns off every present slot on every mesh and, when locked, records "hide everything" (fallback included). "Reset All Textures" turns everything back on and clears the remembered state; the lock stays engaged. Unlocking stops re-applying without changing what is shown. Mesh visibility ("Show Mesh") is not part of the lock.
- **Indicator:** `TexturesHiddenByLock` (locked AND at least one slot remembered off) drives the host's viewport badge — SynthEBD shows "Textures hidden (locked)" top-right.
- **Diagnostics:** one verbose line per batch — `texture visibility lock re-applied to N mesh(es) (scene install | mesh overrides | head-only rebuild | guest overlay | texture overrides); hidden: …; no remembered choice for …` — plus the existing `GLMESH[i]` state dump at scene commit, which prints every mesh's `*Enabled` flags.
...
**Missing detail-map file → slot treated as empty.** If slot 3 names a file that doesn't resolve anywhere in the load order, the mesh gets `HasDetailMap=false` instead of the 1×1 white fallback texture (`ApplyTexturesToGlMesh`). The engine ignores a missing detail map; binding white instead saturates the face under *both* blend modes (legacy overlay with a white top layer → white; engine-style → ×~4 multiply). Real-world trigger: female facegen NIFs baking `textures\actors\character\female\blankdetailmap.dds`, which vanilla only ships in the `male` directory. The broken path is still recorded as a texture source so the hover tooltip shows it as "(not found)", and (for face shapes) the slot then qualifies for the BlankDetailmap debug fallback below.
...
**Wide-line fallback.** `Initialize` probes once whether the context honors `glLineWidth > 1` (`_wideLinesSupported`). A forward-compatible core context — the offscreen renderer's `GameWindow` reports `CONTEXT_FLAGS = FORWARD_COMPATIBLE`, core profile — rejects it with `GL_INVALID_VALUE` and keeps drawing 1 px lines, which all but vanish in a scaled-down thumbnail. There the line channels (measurement lines, region cap-loop contour, region wireframe) are instead expanded on the CPU into screen-aligned quads of the same pixel width: both endpoints are projected through `view * projection` to NDC, offset by half the width perpendicular to the segment *in pixel space* (so the width is exact on non-square viewports), and drawn as two triangles with identity `u_view` / `u_projection` and Z pinned to 0 (depth test is off for the pass anyway; Z = 0 keeps the quad inside the clip range). Back-face culling is suspended for the quads, since their winding follows the segment's screen direction. Segments with an endpoint behind the camera are skipped. The live WPF viewer's context accepts wide lines, so it keeps the `GL.LineWidth` path unchanged.

**The overlay draws into whatever framebuffer the passes before it leave bound**, so any post-process that runs ahead of it must hand back the host's scene FBO. Bloom (`RenderBloom`, run just before the overlay pass when `EnableBloom && EnableToneMapping`) captures `DRAW_FRAMEBUFFER_BINDING` as its restore target, and it must do so **before** `EnsureBloomFbos`: (re)creating the bloom FBOs leaves the last one bound, and a capture after that "restores" to a bloom FBO, so the glow composite and the whole overlay pass land off-screen. The live viewer would only hit that on its first frame or a resize, but the offscreen renderer builds a fresh `GlRenderer` per render, so every bloom-enabled offscreen image would lose both its bloom and its overlay.

`debug.frag` also carries a `u_alpha` output-alpha uniform, added for the [section clip plane](#section-clip-plane-clip-f--clip-b)'s translucent fill. It is `1.0` for every overlay channel here except a translucent region solid (below) — but an unset GLSL uniform is `0`, i.e. fully transparent, so **every** caller of the debug shader must push it rather than relying on a default. The clip-plane visualization enables blending and clears the depth mask, and restores both.

**Measurement-region tint (translucent, depth-tested).** `MeasurementRegionTriangles` holds the surface patches of the regions the displayed measurements use (a RegionVolume's region, or the region behind a Region-strategy key vertex). It is drawn first in the marker pass by `DrawMeasurementRegions`, lit like the region solid but alpha-blended at `MeasurementRegionAlpha` (0.45, depth writes off) with `glBlendFuncSeparate(SRC_ALPHA, ONE_MINUS_SRC_ALPHA, ZERO, ONE)` — the separate alpha factors keep the destination alpha at 1, so an offscreen BGRA readback doesn't come out semi-transparent where the tint sits — and with the depth test re-enabled for this one draw plus `glPolygonOffset(-1, -2)` on fill, because the patch lies exactly on the body surface and would otherwise z-fight. The result is a surface tint: shaded where the patch is visible, hidden on the far side. This works because the overlay pass draws into the scene FBO whose depth buffer still holds the body (the same reason the wireframe overlay can depth-test). It restores depth-off / opaque / depth-writes-on before the region solid and marker spheres draw. It is a separate channel from the region editor's `RegionSolidTriangles` on purpose: the editor clears and rewrites its channel on every preset change and on region deselect, which would wipe a shared tint.
...
| `MeasurementRegionTriangles` | `List<float>` interleaved pos+normal | cyan (`MeasurementRegionColor`) at `MeasurementRegionAlpha` | `DrawMeasurementRegions` (marker pass, lit, **translucent + depth-tested**, drawn first) | `SetMeasurementRegionTint` |
...
The per-request VM is created fresh for every render, so a hook's overlay can't leak into a later request (including the software fallback preview, which shares the renderer). The hook must not touch WPF objects — it isn't on the dispatcher thread — and a throwing hook is logged and costs only the overlay; the frame still renders. SynthEBD's Show Spread window uses it to draw a metric's measurement lines on each thumbnail, and to tint any region the measurement uses (a RegionVolume's region, or the region behind a Region-strategy key vertex) translucent cyan through the measurement-region tint channel (`SetMeasurementRegionTint`), the same one the live viewer's measurement overlay uses. Null `BeforeDraw` leaves the offscreen path byte-identical to before.
