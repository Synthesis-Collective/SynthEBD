...
- [x] **R3 -- `VM_FilePathReplacement` paths deduped onto `FilePathDestinationMap` -- DONE** (commit `8fbad94f`).
  Re-framed: `FilePathDestinationMap` is **already** the canonical path source (each path defined once as a
  `public const string Dest_*`, referenced internally). The real triplication was those consts vs. the same 37
  record-path strings re-typed as literals in `VM_FilePathReplacement` -- the `DestinationDetailAbstractDictionary`
  keys AND the `GetPathFromTypeString` switch returns. Pointed the VM at the consts (74 substitutions; 0 path
  literals remain). **Behaviour-preserving**: the literal->const mapping was generated by value-equality (each
  VM literal verified byte-identical to the const it replaced via a `comm` diff -- all 37 matched), so the VM
  now uses a const whose value === the old literal. Left the friendly-name strings + `DestinationOptions` menu
  as-is (VM display concerns, two name formats, no path dup -- a fuller table-driven menu is a separate UI item).
...
### ✅ RESOLVED — see Bucket 3 §R3 · `VM_FilePathReplacement` — 🔧 modernize (record-path strings triplicated)

**RESOLVED (§R3, commit `8fbad94f`):** the two literal copies in `VM_FilePathReplacement`
(`DestinationDetailAbstractDictionary` keys + `GetPathFromTypeString` returns) now reference the canonical
`FilePathDestinationMap.Dest_*` consts (74 substitutions, value-equality-verified). The `DestinationOptions`
menu + friendly-name strings (two formats, no path dup) are intentionally left -- a fuller table-driven menu is
a separate UI item. Original note follows.
